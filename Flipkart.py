...
st.markdown("<h2 style='text-align: center; background-color: #f1f1f1; color: #035689; 
padding: 8px; margin-bottom: 10px; box-shadow: 0px 2px 5px rgba(0, 0, 0, 0.1); fontfamily: Georgia;'>Flipkart Scraping Tool</h2>", unsafe_allow_html=True)
st.write('##')
# Initialize session state
if 'urls' not in state:
 state.urls = []
num_urls = st.number_input("**Enter the number of URLs:**", value=len(state.urls) + 1, 
min_value=1, step=1)
